...
class DriverTrackingWebSocketService {
  ...
  initialize(server) {
    ...
    console.log(
      "🔌 Driver Tracking WebSocket service initialized at /driver-tracking"
    );
    return this.wss;
  }
  ...
  handleUnsubscribe(ws, orderId) {
    ...
    console.log(
      `📡 Customer ${ws.customerId} unsubscribed from order ${orderId}`
    );
  ...
  handleDisconnect(ws) {
    console.log(
      `📱 Customer ${ws.customerId} disconnected from driver tracking`
    );
  ...
  sendTrackingUpdate(orderId, updateType, payload) {
    ...
    console.log(
      `📡 Sent ${updateType} update for order ${orderId} to ${sentCount} clients`
    );
    return sentCount > 0;
  }
  ...
  async sendRealTrackingData(orderId) {
    ...
    try {
      ...
      const assignment = await DriverAssignment.findOne({
        ...
        "driverId",
        "fullName currentLocation rating phone vehicleInfo profileImage"
      );
      ...
      setTimeout(() => {
        const driverInfo = {
          ...
          vehicle: driver.vehicleInfo
            ? `${driver.vehicleInfo.make || "Vehicle"} ${
                driver.vehicleInfo.model || ""
              } - ${driver.vehicleInfo.plateNumber || "N/A"}`.trim()
            : "Vehicle information not available",
          rating: driver.rating || 4.5,
          profileImage:
            driver.profileImage || "https://via.placeholder.com/100x100",
        };
      ...
      if (driver.currentLocation && driver.currentLocation.coordinates) {
        setTimeout(() => {
          const locationData = {
            ...
            speed: 0,
            timestamp:
              driver.currentLocation.lastUpdated || new Date().toISOString(),
          };
        ...
        console.log(
          `⚠️ No location data available for driver ${driver.fullName} (${driver._id})`
        );
        console.log(`Driver location data:`, driver.currentLocation);

        // Only send status message - no mock location fallback
        setTimeout(() => {
      ...
      setTimeout(() => {
        const statusMap = {
          assigned: {
            ...
            message: "Driver has been assigned and is on the way",
          },
          picked_up: {
            status: "picked_up",
        ...
        const statusInfo =
          statusMap[assignment.status] || statusMap["assigned"];
      ...
      console.error(
        `❌ Error fetching tracking data for order ${orderId}:`,
        error
      );
      this.sendTrackingUpdate(orderId, "error", {
