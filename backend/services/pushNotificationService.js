...
class PushNotificationService {
  ...
  async sendPushNotification(notificationData) {
    try {
      const { to, title, body, data = {}, sound = 'default', badge = null } = notificationData;
  ...
  async sendBulkPushNotifications(notifications) {
    try {
      ...
      for (const notification of notifications) {
        const { to, title, body, data = {}, sound = 'default', badge = null } = notification;
  ...
  async handlePushReceipts(receiptIds) {
    try {
      ...
      console.log(`📬 Push receipt results: ${results.delivered} delivered, ${results.failed} failed`);
  ...
  async schedulePushNotification(notificationData, scheduleTime) {
    try {
      ...
      console.log(`📅 Push notification scheduled for ${scheduleTime.toISOString()}`);
