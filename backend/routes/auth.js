...
router.post(
  "/subscription/pause",
  auth,
  userSubscriptionController.pauseSubscription
);
...
router.post(
  "/subscription/resume",
  auth,
  userSubscriptionController.resumeSubscription
);
...
router.get(
  ...
  auth,
  userSubscriptionController.getDeliveryTracking
);

// POST /api/auth/push-token (protected route) - Register push notification token
router.post("/push-token", auth, async (req, res) => {
  try {
    const { token, deviceId, platform, tokenType = "expo" } = req.body;
    ...
    if (!["expo", "fcm"].includes(tokenType)) {
      return res.status(400).json({
        success: false,
        message: "Invalid tokenType. Must be expo or fcm",
      });
    }
...
router.delete("/push-token", auth, async (req, res) => {
  try {
    const { deviceId } = req.body; // tokenType not required here
