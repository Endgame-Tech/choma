...
CustomerSchema.methods.addPushToken = function (
  token,
  deviceId,
  platform,
  tokenType = "expo"
...
CustomerSchema.methods.getActivePushTokens = function () {
  return this.pushTokens.filter((pt) => pt.isActive).map((pt) => pt.token);
};

CustomerSchema.methods.updatePushTokenActivity = function (
  token,
  isActive = true
) {
