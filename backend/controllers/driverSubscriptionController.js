...
class DriverSubscriptionController {
  ...
  async getMySubscriptionDeliveries(req, res) {
    try {
      ...
      const subscriptionAssignments = await DriverAssignment
        .find({
          driverId,
          'subscriptionInfo.subscriptionId': { $exists: true },
          status: { $in: ['assigned', 'picked_up'] }
        })
        .populate('subscriptionInfo.subscriptionId', 'status frequency nextDeliveryDate dietaryPreferences')
        .populate('subscriptionInfo.mealPlanId', 'planName durationWeeks planDescription')
      ...
      const enrichedAssignments = await Promise.all(
        subscriptionAssignments.map(async (assignment) => {
          ...
          const deliveryHistory = await DriverAssignment
            .find({
              driverId,
              'subscriptionInfo.subscriptionId': assignment.subscriptionInfo.subscriptionId,
              status: 'delivered'
            })
            .sort({ deliveredAt: -1 })
            .limit(5)
          ...
          const avgDeliveryTime = deliveryHistory.length > 0 
            ? deliveryHistory.reduce((sum, h) => {
                const deliveryTime = new Date(h.deliveredAt) - new Date(h.estimatedDeliveryTime);
                return sum + Math.max(0, deliveryTime);
              }, 0) / deliveryHistory.length / (1000 * 60) // Convert to minutes
            : 0;
          ...
          return {
            ...
            relationshipMetrics: {
              ...
              consistency: totalDeliveries > 0 ? Math.max(0, 100 - (avgDeliveryTime / 15) * 100) : 100
            }
          };
        })
      );
      ...
      enrichedAssignments.forEach(assignment => {
        ...
        subscriptionGroups[subId].totalDeliveries += assignment.relationshipMetrics.totalDeliveries;
        ...
        data: {
          ...
          summary: {
            ...
            avgRelationshipScore: Math.round(
              Object.values(subscriptionGroups).reduce((sum, g) => sum + g.relationshipScore, 0) /
  ...
  async getWeeklyDeliverySchedule(req, res) {
    try {
      ...
      const weeklyDeliveries = await DriverAssignment
        .find({
          driverId,
      ...
      const significantOptimizations = Array.from(routeOptimizations.values())
        ...
        .map(opt => {
          ...
          const avgDuration = opt.deliveries.reduce((sum, d) => sum + d.estimatedDuration, 0) / opt.deliveries.length;
          opt.estimatedTimeSaving = Math.round(avgDuration * opt.deliveries.length * 0.2);
          return opt;
        })
      ...
      const weeklyStats = {
        ...
        subscriptionDeliveries: weeklyDeliveries.filter(d => d.subscriptionInfo.subscriptionId).length,
        oneTimeDeliveries: weeklyDeliveries.filter(d => !d.subscriptionInfo.subscriptionId).length,
        uniqueCustomers: new Set(weeklyDeliveries.map(d => d.orderId.customer)).size,
        routeOptimizations: significantOptimizations.length,
        totalDistance: weeklyDeliveries.reduce((sum, d) => sum + d.totalDistance, 0),
        totalEarnings: weeklyDeliveries.reduce((sum, d) => sum + d.totalEarning, 0)
      };
  ...
  async getCustomerSubscriptionTimeline(req, res) {
    try {
      ...
      console.log('📋 Getting customer subscription timeline:', { customerId, subscriptionId });

      // Validate driver has delivered to this customer
      const driverCustomerHistory = await DriverAssignment
        .findOne({
          driverId,
          'orderId': { $exists: true },
      ...
      const mealPlanStructure = await MealPlanAssignment
        .find({ mealPlanId: subscription.mealPlanId._id })
      ...
      const timeline = mealPlanStructure.map((planMeal, index) => {
        ...
        const delivery = deliveryHistory.find(d => 
          d.subscriptionInfo.deliveryDay === (index + 1)
        );
        ...
        return {
          ...
          isInProgress: delivery?.status && ['assigned', 'picked_up'].includes(delivery.status),
      ...
      const progressPercentage = totalSteps > 0 ? Math.round((deliveredSteps / totalSteps) * 100) : 0;
      ...
      res.json({
        ...
        data: {
          ...
          progression: {
            ...
            progressPercentage,
            myDeliveries,
            relationshipStrength: Math.round((myDeliveries / Math.max(deliveredSteps, 1)) * 100)
          }
        }
      });
  ...
  async getSubscriptionMetrics(req, res) {
    try {
      ...
      console.log('📊 Getting subscription metrics for driver:', driverId, 'period:', period);
      ...
      const metricsData = await DriverAssignment.aggregate([
        ...
        },
        {
          $group: {
            ...
            uniqueSubscriptionCustomers: { $addToSet: '$subscriptionInfo.subscriptionId' }
          }
        }
      ]);
      ...
      const subscriptionOnTimeRate = subscriptionData.totalSubscriptionDeliveries > 0 
        ? Math.round((subscriptionData.deliveredSubscriptionOrders / subscriptionData.totalSubscriptionDeliveries) * 100) 
        : 100;

      const oneTimeOnTimeRate = oneTimeData.totalOneTimeDeliveries > 0 
        ? Math.round((oneTimeData.deliveredOneTimeOrders / oneTimeData.totalOneTimeDeliveries) * 100) 
        : 100;
      ...
      // Calculate insights
      const insights = {
        customerRetention: subscriptionData.uniqueSubscriptionCustomers.length > 5 ? 'excellent' : 
                          subscriptionData.uniqueSubscriptionCustomers.length > 2 ? 'good' : 'developing',
        performanceVsOneTime: subscriptionOnTimeRate >= oneTimeOnTimeRate ? 'better' : 'needs_improvement',
        earningsComparison: subscriptionData.totalSubscriptionEarnings >= oneTimeData.totalOneTimeEarnings ? 'higher' : 'lower',
        strengths: [],
        improvementAreas: [],
        recommendations: []
      };
      ...
      // Generate insights
      if (subscriptionOnTimeRate >= 95) insights.strengths.push('excellent_delivery_consistency');
      if (avgDeliveryTimeMinutes <= 5) insights.strengths.push('punctual_deliveries');
      if (subscriptionData.uniqueSubscriptionCustomers.length >= 10) insights.strengths.push('strong_customer_relationships');

      if (subscriptionOnTimeRate < 85) insights.improvementAreas.push('delivery_consistency');
      if (avgDeliveryTimeMinutes > 15) insights.improvementAreas.push('delivery_timing');
        ...
        switch (area) {
          case 'delivery_consistency':
            insights.recommendations.push({
              ...
              priority: 'high',
              action: 'Focus on route planning and time management for subscription deliveries',
          ...
          case 'delivery_timing':
            insights.recommendations.push({
              category: 'timing',
              priority: 'medium',
              action: 'Use area clustering and route optimization for better time management',
        ...
        data: {
          ...
          metrics: {
            subscription: {
              ...
              uniqueCustomers: subscriptionData.uniqueSubscriptionCustomers.length
            },
            oneTime: {
            ...
            comparison: {
              subscriptionEarningsPercentage: Math.round(
                (subscriptionData.totalSubscriptionEarnings / 
                 Math.max(subscriptionData.totalSubscriptionEarnings + oneTimeData.totalOneTimeEarnings, 1)) * 100
              ),
              subscriptionDeliveryPercentage: Math.round(
                (subscriptionData.totalSubscriptionDeliveries / 
                 Math.max(subscriptionData.totalSubscriptionDeliveries + oneTimeData.totalOneTimeDeliveries, 1)) * 100
              )
            }
          },
          insights
  ...
  async updateDeliveryStatus(req, res) {
    try {
      ...
      console.log('📦 Updating delivery status:', { assignmentId, status, driverId });
      ...
      if (assignment.subscriptionInfo.subscriptionId && status === 'delivered') {
