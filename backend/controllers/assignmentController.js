...
const calculateChefSuitability = (chef, order) => {
    ...
    if (order.dietaryRequirements && order.dietaryRequirements.length > 0 && chef.specializations && chef.specializations.length > 0) {
        const matchingSpecializations = chef.specializations.filter(s => order.dietaryRequirements.includes(s));
...
exports.getAvailableChefsForOrder = async (req, res) => {
    try {
        ...
        const order = await Order.findById(orderId).populate('customer', 'address dietaryRequirements');
        if (!order) {
...
exports.autoAssignOrder = async (req, res) => {
    try {
        ...
        if (availableChefs.length === 0) {
            return res.status(400).json({ success: false, message: 'No chefs are available for auto-assignment.' });
        }
...
exports.assignOrderToChef = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(orderId) || !mongoose.Types.ObjectId.isValid(chefId)) {
            return res.status(400).json({ success: false, message: 'Invalid order or chef ID' });
        }
        ...
        if (!chef || chef.status !== 'Active') {
            return res.status(404).json({ success: false, message: 'Chef not found or is not active' });
        }

        // Check if order is already assigned
        const existingDelegation = await OrderDelegation.findOne({ order: orderId });
        if (existingDelegation) {
            return res.status(400).json({
                success: false,
                message: 'Order is already assigned. Please reassign if you want to change the chef.',
                data: {
...
exports.reassignOrder = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(orderId) || !mongoose.Types.ObjectId.isValid(newChefId)) {
            return res.status(400).json({ success: false, message: 'Invalid order or chef ID' });
        }
        ...
        if (!newChef || newChef.status !== 'Active') {
            return res.status(404).json({ success: false, message: 'New chef not found or is not active' });
        }

        const existingDelegation = await OrderDelegation.findOne({ order: orderId });
        if (!existingDelegation) {
            return res.status(400).json({ success: false, message: 'Order was not previously assigned.' });
        }
        ...
        if (oldChefId.toString() === newChefId) {
            return res.status(400).json({ success: false, message: 'Cannot reassign to the same chef.' });
        }
        ...
        try {
            ...
            console.log(`Reassignment notification sent to chef ${newChefId} for order ${orderId}`);
        } catch (notificationError) {
            console.error('Failed to send reassignment notification to chef:', notificationError.message);
