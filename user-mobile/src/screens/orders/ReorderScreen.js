  ...
  const handleReorder = async (order) => {
    try {
      ...
      if (result.success) {
        ...
        await logActivity(
          "reorder",
          `Reordered "${order.planName || order.mealPlan?.planName}"`
        );
        ...
        Alert.alert(
          "Reorder Failed",
          result.error || "Unable to place reorder. Please try again."
        );
      }
    } catch (error) {
  ...
  const renderOrderCard = (order) => (
    <View key={order._id || order.id} style={styles(colors).orderCard}>
      <View style={styles(colors).orderHeader}>
        <Image
          source={
            order.image || order.mealPlan?.image
      ...
      <TouchableOpacity
        style={[
          styles(colors).reorderButton,
          reordering === (order._id || order.id) &&
            styles(colors).reorderButtonDisabled,
        ]}
      ...
      >
        {loading ? (
          ...
          <View style={styles(colors).emptyContainer}>
            ...
            >
              <Text style={styles(colors).browseMealsButtonText}>
                Browse Meal Plans
              </Text>
            </TouchableOpacity>
          </View>
