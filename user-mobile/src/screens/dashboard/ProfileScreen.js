...
import { VALIDATION } from "../../utils/profileConstants";
...
const ProfileScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (user) {
      // Always update editable user when user data changes
      // This ensures phone and address populate when they load from the database
      setEditableUser({
    ...
    debouncedRefresh();
  }, [user, user?.phone, user?.address, user?.fullName, debounce]);
