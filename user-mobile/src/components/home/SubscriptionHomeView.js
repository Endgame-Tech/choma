...
const SubscriptionHomeView = React.memo(
  ({ activeSubscriptions, setShowBrowseMode }) => {
    ...
    const renderTodaysMeals = () => {
      try {
        ...
        const getSubscriptionDay = () => {
          ...
          if (
            primarySubscription.nextDelivery &&
    ...
    return <View style={styles(colors).container}>{renderTodaysMeals()}</View>;
  },
  (prevProps, nextProps) => {
    return (
      prevProps.activeSubscriptions?.length ===
        nextProps.activeSubscriptions?.length &&
