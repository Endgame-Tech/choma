...
const PaymentScreen = ({ route, navigation }) => {
  ...
  const paymentAmounts = paymentService.calculatePaymentAmounts(subscriptionData?.totalPrice || 0);
  const { subtotal, deliveryFee, tax, total } = paymentAmounts;
  ...
  const initializePayment = async () => {
    ...
    try {
      ...
      const reference = `choma-${Date.now()}-${Math.floor(Math.random() * 1000000)}`;
      ...
      const createdSubscriptionId = subscriptionResult.data.data?._id || 
      ...
      if (!createdSubscriptionId) {
        console.error('No subscription ID found in result:', subscriptionResult.data);
        throw new Error('Failed to get subscription ID from created subscription');
      }
      ...
      if (paymentResult.success) {
        ...
        popup.checkout({
          email: user.email,
          amount: total, // v5 expects amount in Naira, not kobo
          reference: reference,
          metadata: {
            customer_id: user.id,
      ...
      // Check for Paystack API errors
        ...
        if (status === 401) {
          ...
          console.error('Paystack API Authentication error:', data?.message || 'Invalid API key');
        } else if (status === 400) {
          errorMessage = `Payment validation error: ${data?.message || 'Invalid payment data'}`;
        } else if (status >= 500) {
          errorMessage = 'Payment service is currently unavailable. Please try again later.';
        } else {
          errorMessage = data?.message || error.message || 'Payment initialization failed';
        }
      } else {
  ...
  const handlePaymentError = (error) => {
    ...
    if (error.response) {
      ...
      if (data?.code === 'INVALID_API_KEY') {
        ...
        console.error('CRITICAL: Paystack API key is invalid. Backend needs configuration update.');
      } else if (status === 401) {
        ...
        errorMessage = data?.message || error.message || 'Payment processing failed';
      }
    ...
    return (
      <SafeAreaView style={styles(colors).container}>
        ...
        <View style={styles(colors).errorContainer}>
          ...
          <Text style={styles(colors).errorTitle}>Payment Information Missing</Text>
          <Text style={styles(colors).errorText}>
            Missing subscription or meal plan data. Please go back and try again.
          </Text>
          <TouchableOpacity 
    ...
    <SafeAreaView style={styles(colors).container}>
          ...
          <View style={styles(colors).orderCard}>
            ...
            <View style={styles(colors).orderOverlay}>
              ...
              <View style={styles(colors).orderMeta}>
                ...
                <Text style={styles(colors).orderMetaText}>
                  Starts: {new Date(subscriptionData.startDate).toLocaleDateString()}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles(colors).section}>
          ...
          <View style={styles(colors).summaryCard}>
            ...
            <View style={styles(colors).summaryRow}>
              ...
              <Text style={[styles(colors).summaryValue, styles(colors).freeText]}>
            ...
            <View style={[styles(colors).summaryRow, styles(colors).totalRow]}>
              ...
              <Text style={styles(colors).totalValue}>₦{total.toLocaleString()}</Text>
            </View>
          </View>
        </View>
          ...
          <View style={styles(colors).paymentMethodCard}>
            <View style={styles(colors).paymentMethodRow}>
              ...
              <View style={styles(colors).paymentMethodInfo}>
                <Text style={styles(colors).paymentMethodTitle}>Card Payment</Text>
                <Text style={styles(colors).paymentMethodSubtitle}>Secured by Paystack</Text>
              </View>
      ...
      <View style={styles(colors).footer}>
        <View style={styles(colors).totalSummary}>
          ...
          <Text style={styles(colors).totalSummaryValue}>₦{total.toLocaleString()}</Text>
        </View>
        
        <TouchableOpacity 
          style={[styles(colors).payButton, isLoading && styles(colors).payButtonDisabled]}
