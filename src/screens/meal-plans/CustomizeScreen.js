...
const CustomizeScreen = ({ route, navigation }) => {
  ...
  // Customization state
  const [selectedPreferences, setSelectedPreferences] = useState(user?.dietaryPreferences || []);
  ...
  const frequencyOptions = [
    { id: 'daily', label: '1 meal/day', description: 'Perfect for lunch or dinner' },
    { id: 'twice_daily', label: '2 meals/day', description: 'Lunch and dinner' },
    { id: 'thrice_daily', label: '3 meals/day', description: 'All meals covered' },
  ];
  ...
  const cookingStyles = [
    { id: 'traditional', label: 'Traditional Nigerian', description: 'Authentic local flavors' },
    { id: 'international', label: 'International Fusion', description: 'Global cuisine styles' },
    { id: 'balanced', label: 'Balanced Mix', description: 'Best of both worlds' },
  ];
  ...
  const nigerianIngredients = [
    // Proteins
    'Fish', 'Chicken', 'Beef', 'Goat meat', 'Turkey', 'Beans', 'Lentils',
    // Staples
    'Rice', 'Yam', 'Plantain', 'Cassava', 'Sweet potato', 'Cocoyam',
    // Vegetables
    'Spinach', 'Waterleaf', 'Ugwu', 'Bitter leaf', 'Scent leaf', 'Tomatoes', 'Onions', 'Peppers',
    // Spices & Seasonings
    'Palm oil', 'Locust beans', 'Curry powder', 'Thyme', 'Ginger', 'Garlic',
    // Allergens
    'Nuts (groundnuts)', 'Dairy', 'Eggs', 'Shellfish'
  ];
  ...
  const loadAvailableMeals = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (mealsArray.length > 0) {
          const transformedMeals = mealsArray.map(meal => ({
            ...
            image: meal.image ? { uri: meal.image } : require('../../assets/images/meal-plans/wellness-hub.jpg'),
            calories: meal.calories || 400,
            protein: meal.protein || 20,
            dietaryTags: meal.dietaryTags || [],
            ingredients: meal.ingredients ? (typeof meal.ingredients === 'string' ? meal.ingredients.split(',').map(i => i.trim()) : meal.ingredients) : [],
      ...
      console.log('No meals from API, meals will be managed through meal plan customization');
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (customization.preferences) {
          ...
          setExcludedIngredients(customization.preferences.excludedIngredients || []);
        }
  ...
  const handleMealSwap = (mealId) => {
    ...
    if (meal) {
      ...
      const alternatives = availableMeals.filter(m => 
        m.id !== mealId && 
        m.type === meal.type &&
        ...
        {DIETARY_PREFERENCES.map((preference) => (
          <TouchableOpacity
            ...
            style={[
              ...
              selectedPreferences.includes(preference) && styles(colors).preferenceChipSelected
            ]}
            onPress={() => handleDietaryPreferenceToggle(preference)}
          >
            <Text style={[
              styles(colors).preferenceText,
              selectedPreferences.includes(preference) && styles(colors).preferenceTextSelected
            ]}>
              {preference}
            </Text>
            {selectedPreferences.includes(preference) && (
              <Ionicons name="checkmark" size={16} color={colors.white} style={styles(colors).preferenceCheck} />
            )}
          </TouchableOpacity>
        ))}
      </View>
  ...
  const renderToppingSection = () => {
    const nigerianToppings = [
      ...
      { id: 'extra_protein', name: 'Extra protein (Fish/Chicken)', price: '+₦800' },
    ...
    return (
      <View style={styles(colors).section}>
        ...
        <Text style={styles(colors).sectionSubtitle2}>Add extras to your meal</Text>
          ...
          {nigerianToppings.map((topping) => (
            <TouchableOpacity
              ...
              style={[
                ...
                selectedToppings.includes(topping.id) && styles(colors).toppingItemSelected
              ]}
              onPress={() => {
            ...
            >
              <Text style={[
                ...
                selectedToppings.includes(topping.id) && styles(colors).toppingNameSelected
              ]}>
                {topping.name}
              </Text>
              <Text style={[
                styles(colors).toppingPrice,
                selectedToppings.includes(topping.id) && styles(colors).toppingPriceSelected
              ]}>
                {topping.price}
              </Text>
            </TouchableOpacity>
        ...
        {nigerianDietaryConditions.map((condition) => (
          <TouchableOpacity
            ...
            style={[
              ...
              selectedDietaryConditions.includes(condition.id) && styles(colors).conditionChipSelected
            ]}
            onPress={() => {
              const isSelected = selectedDietaryConditions.includes(condition.id);
              if (isSelected) {
                setSelectedDietaryConditions(prev => prev.filter(id => id !== condition.id));
              } else {
          ...
          >
            ...
            <Text style={[
              ...
              selectedDietaryConditions.includes(condition.id) && styles(colors).conditionTextSelected
            ]}>
              {condition.name}
            </Text>
            {selectedDietaryConditions.includes(condition.id) && (
              <Ionicons name="checkmark" size={16} color={colors.white} style={styles(colors).conditionCheck} />
            )}
          </TouchableOpacity>
        ))}
      </View>
          ...
          <ScrollView style={styles(colors).modalScroll}>
            <View style={styles(colors).conditionModalContent}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setShowConditionModal(false)}
              >
                <Text style={styles(colors).conditionModalButtonText}>Got it!</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
        ...
        <View style={styles(colors).modalContent}>
          ...
          <ScrollView style={styles(colors).modalScroll}>
            {alternativeMeals.map((meal) => (
              ...
              >
                <Image source={meal.image} style={styles(colors).alternativeMealImage} />
                <View style={styles(colors).alternativeMealInfo}>
                  <Text style={styles(colors).alternativeMealName}>{meal.name}</Text>
                  ...
                  <View style={styles(colors).alternativeMealTags}>
                    {meal.dietaryTags.map((tag, index) => (
                      <Text key={index} style={styles(colors).alternativeMealTag}>{tag}</Text>
                    ))}
                  </View>
                </View>
    ...
    return (
      <SafeAreaView style={styles(colors).container}>
        <View style={styles(colors).loadingContainer}>
          ...
          <Text style={styles(colors).loadingText}>Loading customization options...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles(colors).container}>
      <StandardHeader 
        ...
        onRightPress={() => {/* Add help functionality */}}
      />
      ...
      <View style={styles(colors).bottomActions}>
        <TouchableOpacity
          style={[styles(colors).continueButton, saving && styles(colors).continueButtonDisabled]}
