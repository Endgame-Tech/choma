...
const HelpSupportScreen = ({ navigation }) => {
        ...
        setFaqs([
          {
            ...
            answer: 'You can place an order by browsing our meal plans, selecting your preferred plan, customizing it to your needs, and proceeding to checkout. We accept various payment methods for your convenience.',
            category: 'orders'
          },
          {
            id: 2,
            question: 'What are your delivery hours?',
            answer: 'We deliver Monday to Sunday from 8:00 AM to 8:00 PM. You can schedule your delivery for a convenient time during checkout.',
            category: 'delivery'
          },
          {
            id: 3,
            question: 'Can I cancel or modify my order?',
            answer: 'You can cancel or modify your order up to 2 hours before the scheduled delivery time. Go to your Orders section to make changes.',
            category: 'orders'
          },
          {
            id: 4,
            question: 'Do you offer vegetarian/vegan options?',
            answer: 'Yes! We have a wide variety of vegetarian and vegan meal plans. You can filter by dietary preferences when browsing our menu.',
            category: 'menu'
          },
          {
            id: 5,
            question: 'How do I track my order?',
            answer: 'Once your order is confirmed, you can track it in real-time through the Orders section in your profile. You\'ll receive notifications about status updates.',
            category: 'delivery'
          }
        ]);
      }
  ...
  const handleWhatsApp = () => {
    Linking.openURL('whatsapp://send?phone=2348000000000&text=Hi, I need help with choma');
  };
        ...
        <View style={styles(colors).faqContainer}>
          {faqs.map((faq, index) => (
            <View key={faq.id || index} style={styles(colors).faqItem}>
              ...
              >
                <Text style={styles(colors).faqQuestionText}>{faq.question}</Text>
                <Ionicons
    ...
    <View style={styles(colors).tabContent}>
      ...
      <View style={styles(colors).section}>
        ...
        <View style={styles(colors).contactOptions}>
          <TouchableOpacity style={styles(colors).contactOption} onPress={handlePhoneCall}>
            <View style={styles(colors).contactIconContainer}>
              <Ionicons name="call" size={24} color={colors.primary} />
            </View>
            <Text style={styles(colors).contactOptionText}>Call Us</Text>
            <Text style={styles(colors).contactOptionSubtext}>+234 800 000 0000</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles(colors).contactOption} onPress={handleEmailSupport}>
            <View style={styles(colors).contactIconContainer}>
              <Ionicons name="mail" size={24} color={colors.primary} />
            </View>
            <Text style={styles(colors).contactOptionText}>Email Us</Text>
            <Text style={styles(colors).contactOptionSubtext}>support@choma.ng</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles(colors).contactOption} onPress={handleWhatsApp}>
            <View style={styles(colors).contactIconContainer}>
              <Ionicons name="logo-whatsapp" size={24} color={colors.success} />
            </View>
            <Text style={styles(colors).contactOptionText}>WhatsApp</Text>
            <Text style={styles(colors).contactOptionSubtext}>Chat with us</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles(colors).section}>
        ...
        <View style={styles(colors).form}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.alert(
                    ...
                    [
                      { text: 'General', onPress: () => setContactForm(prev => ({...prev, category: 'general'})) },
                      { text: 'Order Issues', onPress: () => setContactForm(prev => ({...prev, category: 'orders'})) },
                      { text: 'Payment', onPress: () => setContactForm(prev => ({...prev, category: 'payment'})) },
                      { text: 'Delivery', onPress: () => setContactForm(prev => ({...prev, category: 'delivery'})) },
                      { text: 'Account', onPress: () => setContactForm(prev => ({...prev, category: 'account'})) },
                    ]
                  );
                }}
              >
                <Text style={styles(colors).pickerText}>
                  {contactForm.category.charAt(0).toUpperCase() + contactForm.category.slice(1)}
                </Text>
          ...
          <TouchableOpacity
            style={[styles(colors).submitButton, submitting && styles(colors).submitButtonDisabled]}
  ...
  const renderTicketsTab = () => (
    <View style={styles(colors).tabContent}>
      {ticketsLoading ? (
        <View style={styles(colors).loadingContainer}>
          <ActivityIndicator size="large" color={colors.primary} />
          <Text style={styles(colors).loadingText}>Loading your tickets...</Text>
        </View>
      ) : tickets.length > 0 ? (
        <View style={styles(colors).ticketsContainer}>
          {tickets.map((ticket) => (
            <View key={ticket._id || ticket.id} style={styles(colors).ticketItem}>
              <View style={styles(colors).ticketHeader}>
                <Text style={styles(colors).ticketSubject}>{ticket.subject}</Text>
                <View style={[
                  styles(colors).ticketStatus,
                  { backgroundColor: ticket.status === 'open' ? colors.warning + '20' : colors.success + '20' }
                ]}>
                  <Text style={[
                    styles(colors).ticketStatusText,
                    { color: ticket.status === 'open' ? colors.warning : colors.success }
                  ]}>
