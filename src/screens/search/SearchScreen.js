...
const SearchScreen = ({ navigation }) => {
  ...
  const hasActiveFilters = Object.keys(activeFilters).some(key => {
    ...
    if (key === 'priceRange') return value && (value[0] > 0 || value[1] < 50000);
    ...
    try {
      ...
      try {
        ...
        if (response?.success && Array.isArray(response.data)) {
          ...
          setPopularSearches(['Jollof Rice', 'Amala & Ewedu', 'Pepper Soup', 'Suya', 'Pounded Yam']);
        }
      } catch (error) {
  ...
  const performLocalSearch = (query, filters) => {
    ...
    if (filters.sortBy) {
      switch (filters.sortBy) {
        case 'price-low':
          results.sort((a, b) => (a.totalPrice || a.price || 0) - (b.totalPrice || b.price || 0));
          break;
        case 'price-high':
          results.sort((a, b) => (b.totalPrice || b.price || 0) - (a.totalPrice || a.price || 0));
          break;
        case 'newest':
  ...
  const getActiveFiltersCount = () => {
    ...
    if (activeFilters.priceRange && (activeFilters.priceRange[0] > 0 || activeFilters.priceRange[1] < 50000)) count++;
    ...
    <SafeAreaView style={styles(colors).container}>
      ...
      <View style={styles(colors).searchContainer}>
        <View style={styles(colors).searchInputContainer}>
          <Ionicons name="search" size={20} color={colors.textMuted} style={styles(colors).searchIcon} />
          <TextInput
          ...
          {searchQuery.length > 0 && (
            <TouchableOpacity onPress={clearSearch} style={styles(colors).clearButton}>
        ...
        <View style={styles(colors).activeFiltersContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles(colors).filtersRow}>
              {activeFilters.audiences && activeFilters.audiences.length > 0 && (
                <View style={styles(colors).activeFilterChip}>
                  <Text style={styles(colors).activeFilterText}>
                    {activeFilters.audiences.length} Audience{activeFilters.audiences.length > 1 ? 's' : ''}
                  </Text>
              ...
              {activeFilters.priceRange && (activeFilters.priceRange[0] > 0 || activeFilters.priceRange[1] < 50000) && (
                <View style={styles(colors).activeFilterChip}>
                  <Text style={styles(colors).activeFilterText}>
              ...
              >
                <Text style={styles(colors).clearAllFiltersText}>Clear All</Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
      ...
      {/* Search Results */}
      <ScrollView style={styles(colors).resultsContainer} contentContainerStyle={styles(colors).scrollContent}>
        {searchQuery.length > 0 ? (
          isSearching ? (
            ...
            <View>
              ...
              <View style={styles(colors).resultsGrid}>
                {searchResults.map((plan) => (
                  ...
                  >
                    <Image 
                      source={plan.planImage || plan.image ? { uri: plan.planImage || plan.image } : require('../../assets/images/meal-plans/fitfuel.jpg')}
                      style={styles(colors).resultImage}
                    />
                    <TouchableOpacity 
                      style={styles(colors).resultHeartButton}
                      onPress={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      <Ionicons 
                        ...
                        size={16} 
                        color={isBookmarked(plan.id || plan._id) ? colors.error : colors.white} 
                      />
                    </TouchableOpacity>
                    <View style={styles(colors).resultContent}>
                      <Text style={styles(colors).resultTitle} numberOfLines={2}>{plan.planName || plan.name}</Text>
                      <Text style={styles(colors).resultCategory}>{plan.targetAudience || plan.category}</Text>
                      <Text style={styles(colors).resultPrice}>₦{(plan.totalPrice || plan.basePrice || plan.price || 0).toLocaleString()}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
            ...
            <View style={styles(colors).emptyResults}>
              ...
              <Text style={styles(colors).emptyTitle}>
                {hasActiveFilters ? 'No plans match your filters' : 'No results found'}
              </Text>
              ...
              {hasActiveFilters && (
                <TouchableOpacity 
                  ...
                  onPress={clearAllFilters}
                >
                  <Text style={styles(colors).clearFiltersButtonText}>Clear Filters</Text>
                </TouchableOpacity>
              )}
            </View>
          )
          ...
          <View style={styles(colors).suggestionsContainer}>
            {historyLoading ? (
              <View style={styles(colors).loadingContainer}>
                ...
                <Text style={styles(colors).loadingText}>Loading suggestions...</Text>
              </View>
            ) : (
              <>
                {/* Search History */}
                {searchHistory.length > 0 && (
                  <View style={styles(colors).historySection}>
                    <Text style={styles(colors).suggestionsTitle}>Recent Searches</Text>
                    {searchHistory.map((historyItem, index) => (
                      ...
                      >
                        ...
                        <Text style={styles(colors).suggestionText}>{historyItem}</Text>
                ...
                <View style={styles(colors).popularSection}>
                  <Text style={styles(colors).suggestionsTitle}>Popular Meal Plans</Text>
                  <View style={styles(colors).resultsGrid}>
                    {(Array.isArray(popularSearches) ? popularSearches : []).map((plan, index) => (
                      <TouchableOpacity
                      ...
                      >
                        <Image 
                          source={plan.image ? { uri: plan.image } : require('../../assets/images/meal-plans/fitfuel.jpg')}
                          style={styles(colors).resultImage}
                        />
                        <View style={styles(colors).resultContent}>
                          <Text style={styles(colors).resultTitle} numberOfLines={2}>{plan.planName || plan.name}</Text>
                          <Text style={styles(colors).resultCategory}>{plan.category}</Text>
                          <Text style={styles(colors).resultPrice}>₦{(plan.price || 0).toLocaleString()}</Text>
                        </View>
                      </TouchableOpacity>
                    ))}
                  </View>
