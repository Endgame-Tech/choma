...
const DeliveryManagementScreen = ({ navigation }) => {
  ...
  const filteredDeliveries = deliveries.filter(delivery => {
    ...
    const matchesSearch = delivery.trackingId.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    >
      <View style={styles(colors).cardHeader}>
        <View style={styles(colors).orderInfo}>
          ...
          <Text style={styles(colors).orderDate}>{formatDate(delivery.createdAt)}</Text>
        </View>
        <View style={[styles(colors).statusBadge, { backgroundColor: getStatusColor(delivery.deliveryStatus) }]}>
          <Text style={styles(colors).statusText}>{delivery.deliveryStatus}</Text>
        </View>
      </View>

      <View style={styles(colors).cardContent}>
        <View style={styles(colors).orderDetails}>
          <Text style={styles(colors).orderAmount}>₦{delivery.order.totalAmount?.toLocaleString()}</Text>
          <Text style={styles(colors).deliveryAddress}>{delivery.deliveryLocation?.address}</Text>
        </View>

        {delivery.driver && (
          <View style={styles(colors).driverInfo}>
            <Ionicons name="person-circle-outline" size={20} color={colors.primary} />
            <Text style={styles(colors).driverName}>{delivery.driver.fullName}</Text>
            <View style={styles(colors).driverRating}>
              <Ionicons name="star" size={12} color={colors.warning} />
              <Text style={styles(colors).ratingText}>{delivery.driver.rating}</Text>
            </View>
          </View>
        )}
      ...
      <View style={styles(colors).cardFooter}>
        ...
        {delivery.deliveryStatus === 'Delivered' && !delivery.customerRating && (
    ...
    return (
      <SafeAreaView style={styles(colors).container}>
        <View style={styles(colors).loadingContainer}>
          ...
          <Text style={styles(colors).loadingText}>Loading your deliveries...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles(colors).container}>
      ...
      >
        ...
        <View style={styles(colors).headerContent}>
          ...
          <Text style={styles(colors).headerSubtitle}>Track all your orders</Text>
        </View>
      </LinearGradient>
        ...
        {statusFilters.map((filter) => (
          ...
          >
            <Text style={[
              ...
              selectedStatus === filter.id && styles(colors).filterChipTextActive
            ]}>
              {filter.label}
            </Text>
          </TouchableOpacity>
