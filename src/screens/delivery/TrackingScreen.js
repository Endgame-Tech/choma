...
export const TrackingScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (trackingId) {
      ...
      refreshInterval.current = setInterval(loadTrackingData, 30000);
    }
    ...
    return (
      <SafeAreaView style={styles(colors).container}>
        <View style={styles(colors).loadingContainer}>
          ...
          <Text style={styles(colors).loadingText}>Loading tracking information...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        <View style={styles(colors).statusCard}>
          <View style={styles(colors).statusHeader}>
            <View style={[styles(colors).statusIcon, { backgroundColor: getStatusColor(tracking.deliveryStatus) }]}>
              <Ionicons 
            ...
            <View style={styles(colors).statusInfo}>
              <Text style={styles(colors).statusText}>{tracking.deliveryStatus}</Text>
              <Text style={styles(colors).statusTime}>
                {tracking.timeline.length > 0 && formatTime(tracking.timeline[tracking.timeline.length - 1].timestamp)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles(colors).driverCard}>
            ...
            <View style={styles(colors).driverInfo}>
              ...
              <View style={styles(colors).driverDetails}>
                <Text style={styles(colors).driverName}>{tracking.driver.fullName}</Text>
                <View style={styles(colors).driverRating}>
                  <Ionicons name="star" size={14} color={colors.warning} />
                  <Text style={styles(colors).ratingText}>{tracking.driver.rating || 5.0}</Text>
                </View>
                <Text style={styles(colors).vehicleInfo}>
                  {tracking.driver.vehicleInfo?.type} • {tracking.driver.vehicleInfo?.plateNumber}
                </Text>
              </View>
              <TouchableOpacity onPress={callDriver} style={styles(colors).callButton}>
            ...
            {tracking.timeline.map((event, index) => (
              <View key={index} style={styles(colors).timelineItem}>
                ...
                <View style={styles(colors).timelineContent}>
                  <Text style={styles(colors).timelineStatus}>{event.status}</Text>
                  <Text style={styles(colors).timelineTime}>
                    {formatDate(event.timestamp)}
                  </Text>
                  {event.notes && (
                    <Text style={styles(colors).timelineNotes}>{event.notes}</Text>
                  )}
                </View>
              </View>
            ))}
        ...
        <View style={styles(colors).orderCard}>
          ...
          <View style={styles(colors).orderInfo}>
            <View style={styles(colors).infoRow}>
              ...
              <Text style={styles(colors).infoValue}>#{tracking.order._id?.slice(-6)}</Text>
            </View>
            <View style={styles(colors).infoRow}>
              <Text style={styles(colors).infoLabel}>Delivery Address:</Text>
              <Text style={styles(colors).infoValue}>{tracking.deliveryLocation.address}</Text>
            </View>
            {tracking.deliveryLocation.instructions && (
              <View style={styles(colors).infoRow}>
                <Text style={styles(colors).infoLabel}>Instructions:</Text>
                <Text style={styles(colors).infoValue}>{tracking.deliveryLocation.instructions}</Text>
              </View>
            )}
          </View>
        </View>
        ...
        {tracking.deliveryStatus === 'Delivered' && !tracking.customerRating && (
          <TouchableOpacity 
            style={styles(colors).rateButton}
            onPress={() => setShowRatingModal(true)}
          >
            <Text style={styles(colors).rateButtonText}>Rate Your Delivery</Text>
          </TouchableOpacity>
        )}
      </ScrollView>
        ...
        <View style={styles(colors).modalOverlay}>
          <View style={styles(colors).modalContent}>
            ...
            <View style={styles(colors).modalButtons}>
              <TouchableOpacity 
                style={[styles(colors).modalButton, styles(colors).cancelButton]}
              ...
              <TouchableOpacity 
                style={[styles(colors).modalButton, styles(colors).submitButton]}
