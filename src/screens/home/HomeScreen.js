...
const HomeScreen = ({ navigation }) => {
  ...
  const confirmPauseSubscription = async () => {
    ...
    try {
      ...
      const result = await apiService.updateSubscription(activeSubscription._id, {
        status: 'paused',
        pauseReason,
        pauseDuration,
    ...
    try {
      ...
      if (result.success) {
        ...
        if (Array.isArray(subscriptions) && subscriptions.length > 0) {
          ...
          if (subscriptions && (status === 'active' || status === 'paid' || subscriptions.paymentStatus === 'Paid')) {
  ...
  const renderTodaysMeals = () => {
    ...
    console.log('Week number:', weekNumber, '-> Mapped to week:', currentWeekNumber);
    console.log('Day in week:', dayInWeek);
    console.log('Looking for meals in: week' + currentWeekNumber + '.' + dayName);
    console.log('Weekly meals structure:', JSON.stringify(weeklyMeals, null, 2));
    console.log('Current week data:', JSON.stringify(currentWeek, null, 2));
    console.log('Today\'s meals data:', JSON.stringify(todaysMealsData, null, 2));
    ...
    console.log('\n=== MEAL IMAGES DEBUG ===');
    console.log('Full meal images structure:', JSON.stringify(mealImages, null, 2));
    console.log('Today meal images for', dayName, ':', JSON.stringify(todayMealImages, null, 2));
    ...
    const getMealImage = (mealType) => {
      ...
      if (activeSubscription.mealPlanId.planImageUrl) {
        console.log('Using plan image URL as fallback:', activeSubscription.mealPlanId.planImageUrl);
    ...
    const getMealName = (mealType) => {
      ...
      if (mealData) {
        ...
        if (typeof mealData === 'string') {
          ...
          return mealData.name || mealData.meal || mealData.title || mealData.description;
        }
      }
      return null;
    };
    ...
    const availableMeals = Object.entries(todaysMeals).filter(([_, meal]) => meal.name);
    
    console.log('Available meals for today:', availableMeals.map(([type, meal]) => `${type}: ${meal.name}`));
    
    // If no meals are available for today, show a message
    if (availableMeals.length === 0) {
      return (
        <View style={styles(colors).todaysMealsSection}>
          <Text style={styles(colors).sectionTitle}>Today's Meals - Day {subscriptionDay}</Text>
          <View style={styles(colors).noMealsContainer}>
            <Ionicons name="restaurant-outline" size={48} color={colors.textMuted} />
            <Text style={styles(colors).noMealsTitle}>No meals scheduled for today</Text>
            <Text style={styles(colors).noMealsText}>
              Week {currentWeekNumber}, {dayName} meals are not available in your meal plan. Please contact support.
            </Text>
            <TouchableOpacity 
              style={styles(colors).contactSupportButton}
              onPress={() => setShowBrowseMode(true)}
            >
              <Text style={styles(colors).contactSupportText}>Browse Meal Plans</Text>
            </TouchableOpacity>
          </View>
        </View>
      );
    ...
    return (
      <View style={styles(colors).todaysMealsSection}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles(colors).mealsScroll}>
          {Object.entries(todaysMeals).map(([mealType, meal]) => (
            <TouchableOpacity key={mealType} style={styles(colors).todayMealCard}>
              <View style={styles(colors).todayMealImageContainer}>
                <Image source={meal.image} style={styles(colors).todayMealImage} />
                <LinearGradient
                ...
                >
                  ...
                  <Text style={styles(colors).mealType}>{mealType.charAt(0).toUpperCase() + mealType.slice(1)}</Text>
                  <Text style={styles(colors).todayMealName} numberOfLines={2}>{meal.name}</Text>
                  <Text style={styles(colors).mealCalories}>{meal.calories} cal</Text>
                </LinearGradient>
              </View>
            </TouchableOpacity>
          ))}
  ...
  const renderSubscriptionStatus = () => {
    ...
    const progressPercentage = totalDuration > 0 ? Math.min(100, Math.max(0, (daysElapsed / totalDuration) * 100)) : 0;
            ...
            {menuOptions.map((option, index) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  index === menuOptions.length - 1 && styles(colors).menuOptionLast
                ]}
        ...
        <View style={styles(colors).modalOverlay}>
          <View style={styles(colors).pauseModal}>
              ...
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    pauseDuration === duration.value && styles(colors).durationOptionSelected
                  ]}
                  onPress={() => setPauseDuration(duration.value)}
                >
                  <Text style={[
                    styles(colors).durationOptionText,
                    pauseDuration === duration.value && styles(colors).durationOptionTextSelected
                  ]}>
                    {duration.label}
                  </Text>
                </TouchableOpacity>
            ...
            <View style={styles(colors).modalActions}>
              <TouchableOpacity
                style={[styles(colors).modalButton, styles(colors).modalButtonSecondary]}
                onPress={() => setShowPauseModal(false)}
              >
                <Text style={styles(colors).modalButtonTextSecondary}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
              ...
              >
                <Text style={styles(colors).modalButtonTextPrimary}>Pause Subscription</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Ionicons 
              name={isBookmarked(plan.id || plan._id) ? "heart" : "heart-outline"} 
              size={20} 
              color={isBookmarked(plan.id || plan._id) ? colors.error : colors.black} 
            />
          </TouchableOpacity>
          ...
          <Text style={styles(colors).popularCardDescription} numberOfLines={2}>
            {plan.description || plan.subtitle || plan.shortDescription || 'Delicious and nutritious meal plan crafted for your healthy lifestyle'}
          </Text>
        ...
        <View style={styles(colors).mealplanImageContainer}>
          ...
          >
            ...
            <Text style={styles(colors).mealplanDescription} numberOfLines={2}>
              {plan.description || plan.subtitle || plan.shortDescription || 'Satisfy your junk food cravings with fast, delicious, and effortless delivery.'}
            </Text>
        ...
        <View style={styles(colors).heroBanner}>
          <View style={styles(colors).heroContent}>
            <View style={styles(colors).heroTextSection}>
              ...
              <Text style={styles(colors).heroSubtitle}>Grab our mouthwatering burger deal before it's gone!</Text>
              <TouchableOpacity 
                style={styles(colors).heroButton}
                activeOpacity={0.8}
                onPress={() => navigation.navigate('Search')}
              >
                <Text style={styles(colors).heroButtonText}>Get 20% Off 🎉</Text>
                <Ionicons name="chevron-forward" size={16} color="#000" style={styles(colors).heroButtonIcon} />
              </TouchableOpacity>
            </View>
  ...
  return (
    <SafeAreaView style={styles(colors).container}>
      ...
      <View style={styles(colors).header}>
        <Text style={styles(colors).headerTitle}>MyChef</Text>
              ...
              <>
                ...
                <View style={styles(colors).backToPlanSection}>
                  ...
                  >
                    ...
                    <Text style={styles(colors).backToPlanText}>Back to My Plan</Text>
                  </TouchableOpacity>
                </View>
                ...
                <View style={styles(colors).section}>
                  <View style={styles(colors).sectionHeader}>
                    <Text style={styles(colors).sectionTitle}>Popular plans</Text>
                  ...
                  {loading && !refreshing && (
                    <View style={styles(colors).mealplansGrid}>
                      {[...Array(4)].map((_, index) => <MealCardSkeleton key={index} />)}
                    </View>
                  )}
                    ...
                    <View style={styles(colors).popularFoodContainer}>
                      {displayPlans.length > 0 ? (
                        <ScrollView
                          ...
                          contentContainerStyle={styles(colors).popularFoodScrollContent}
                          scrollEventThrottle={16}
                          decelerationRate="fast"
                        >
                          {displayPlans.map((plan, index) => renderPopularFoodCard(plan, index))}
                        </ScrollView>
                      ) : (
                        <View style={styles(colors).emptyContainer}>
                          ...
                          <Text style={styles(colors).emptyText}>
                            Try selecting a different category or refresh to load plans.
                          </Text>
                        </View>
                      )}
                    </View>
                ...
                <View style={styles(colors).section}>
                  ...
                  {!loading && !error && (
                    <View style={styles(colors).mealplansGrid}>
                      {displayPlans.slice(0, 4).map((plan, index) => renderMealplanCard(plan, index))}
                    </View>
                  )}
                </View>
              </>
          ...
          if (!subscriptionLoading && activeSubscription) {
            ...
            <View style={styles(colors).quickStatsSection}>
              ...
              <View style={styles(colors).statsGrid}>
                ...
                <View style={styles(colors).statCard}>
                  <Text style={styles(colors).statNumber}>
                    ₦{(activeSubscription.metrics?.totalSpent || activeSubscription.totalPrice || 0).toLocaleString()}
                  </Text>
                  <Text style={styles(colors).statLabel}>Total Spent</Text>
                </View>
                <View style={styles(colors).statCard}>
                  <Text style={styles(colors).statNumber}>
                    {activeSubscription.metrics?.consecutiveDays || Math.max(0, daysElapsed)}
                  </Text>
            ...
            <View style={styles(colors).browseSection}>
              ...
              >
                ...
                <Text style={styles(colors).browseMealPlansText}>Browse Meal Plans</Text>
