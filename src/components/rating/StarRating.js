...
const StarRating = ({
  ...
  const handlePress = useCallback((starIndex, isHalf = false) => {
  ...
  const handlePressIn = useCallback((starIndex, isHalf = false) => {
    if (disabled || readOnly) return;
  ...
  const renderStar = (index) => {
    ...
    const iconName = isFullyFilled ? 'star' : isHalfFilled ? 'star-half' : 'star-outline';
