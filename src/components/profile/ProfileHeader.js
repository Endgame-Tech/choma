...
const ProfileHeader = ({ 
  ...
  const formatUserName = (fullName) => {
    ...
    if (names.length === 1) {
      ...
      return { firstName: name.length > 12 ? name.substring(0, 12) + '...' : name, lastName: '' };
      ...
      return { 
        firstName: firstName.length > 12 ? firstName.substring(0, 12) + '...' : firstName,
        lastName: lastName.length > 12 ? lastName.substring(0, 12) + '...' : lastName
      };
    } else {
      ...
      <View style={styles(colors).imageBackground}>
        {profileImage || user?.profileImage ? (
          <Image 
            ...
            onError={() => console.log('Profile background image failed to load')}
          />
        ) : (
